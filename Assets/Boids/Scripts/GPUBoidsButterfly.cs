...
{
    public class GPUBoidsButterfly : MonoBehaviour
    {
        // Boidデータの構造体
        [System.Serializable]
        struct BoidData
        ...
        public float CircleWeight = 0.0f;

        public float CylinderWeight = 0.0f;
        public float SphereWeight = 0.0f;
        public float SphereRadius = 2.0f;
        public float SphereCeil = 16.0f;
        public float IniPosY = 0.0f;
        ...
        void OnDrawGizmos()
        {
            ...
            if (WallSize.y == 0 || WallSize.z == 0)
                Gizmos.DrawWireSphere(WallCenter, WallSize.x);
            else
        ...
        void InitBuffer()
        {
            ...
            for (var i = 0; i < MaxObjectNum; i++)
            {
                ...
                boidDataArr[i].Position = Random.insideUnitSphere * 1.2f;
                boidDataArr[i].Position.y += IniPosY;
        ...
        void Simulation()
        {
            ...
            cs.SetVector("_SphereCenter", WallCenter);
            cs.SetVector("_WallSize", WallSize);
            cs.SetFloat("_AvoidWallWeight", AvoidWallWeight);
            cs.SetFloat("_CircleWeight", CircleWeight);
            cs.SetFloat("_CylinderWeight", CylinderWeight);
            cs.SetFloat("_SphereWeight", SphereWeight);
            cs.SetFloat("_SphereRadius", SphereRadius);
            cs.SetFloat("_SphereCeil", SphereCeil);
            cs.SetFloat("_ForceY", ForceY);
            cs.SetVector("_CamPos", cam);
            cs.SetInt("_IsFreeze", IsFreeze);
